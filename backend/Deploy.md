...
```bash
gcloud secrets create vaani-sa-key \
  --data-file=secrets/serviceAccountKey.json \
  --project=autocalltranslate

# Grant Cloud Run's SA access to the secret
gcloud secrets add-iam-policy-binding vaani-sa-key \
  --member="serviceAccount:vaani-backend@autocalltranslate.iam.gserviceaccount.com" \
...
```bash
gcloud artifacts repositories create vaani \
...
```bash
PROJECT_NUMBER=$(gcloud projects describe autocalltranslate --format='value(projectNumber)')
...
gcloud builds submit --project=autocalltranslate
...
gcloud run deploy vaani-backend \
  --image=asia-south1-docker.pkg.dev/autocalltranslate/vaani/backend:<COMMIT_SHA> \
  --region=asia-south1 \
  --project=autocalltranslate
```
...
| `GOOGLE_CLOUD_PROJECT` | `autocalltranslate` |
...
| `GOOGLE_CLOUD_PROJECT` | `autocalltranslate` | Yes |
